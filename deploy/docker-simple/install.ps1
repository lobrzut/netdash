...
if (-not (Test-Path ".env")) {
    $secure = Read-Host "Haslo administratora NetDash" -AsSecureString
    $adminPass = [Runtime.InteropServices.Marshal]::PtrToStringAuto(
        [Runtime.InteropServices.Marshal]::SecureStringToBSTR($secure))
    if ([string]::IsNullOrWhiteSpace($adminPass)) { throw "Haslo nie moze byc puste." }
...
NETDASH_DEFAULT_ADMIN_PASSWORD=$adminPass
NETDASH_DEFAULT_ADMIN_USER=admin
NETDASH_SCAN_CIDR=192.168.1.0/24
"@ | Set-Content -Encoding utf8 ".env"
    Write-Host "-> Utworzono .env (NETDASH_SCAN_CIDR dla Docker Desktop)" -ForegroundColor Green
} else {
